...
DepthImageAverager::DepthImageAverager(int width, int height, int min_elements,
                                       int max_elements) {
  width_ = width;
  height_ = height;
...
void DepthImageAverager::vector2DepthImage_(sensor_msgs::ImagePtr &image,
...
void DepthImageAverager::reset() { size_ = 0; }

//////////////////////////////////////////////////////////////////////////

int DepthImageAverager::size() { return size_; }
...
int DepthImageAverager::computeMedian(sensor_msgs::ImagePtr &averaged_image,
                                      bool true_median) {
  if (size_ <= 0) return 1;
  ...
  for (int i = 0; i < width_ * height_; i++) {
    ...
    if (true_median && filtered_size % 2 == 0) {
      // true median
      std::nth_element(tmp_buffer.begin(), m - 1,
                       tmp_buffer.begin() + filtered_size);
...
int DepthImageAverager::computeMAD(sensor_msgs::ImagePtr &averaged_image,
                                   float mad_upper_limit_a,
                                   float mad_upper_limit_b, float mad_scale,
                                   bool true_median) {
  if (size_ <= 0) return 1;
  ...
  for (int i = 0; i < width_ * height_; i++) {
    ...
    for (int j = 0; j < filtered_size; j++) {
      ...
      if (std::isfinite(val) &&
          std::fabs(val - median_value) < mad_scale * mad) {
        mad_mean += val;
        mad_size++;
      }
    }
    ...
    averaged_arr[i] =
        mad_mean;  // TODO: maybe a weighted avg between the mad_mean and the
                   // median value? needs more investigation.
  }
...
cl_int DepthImageAverager::initOpenCL_() {
  ...
  if (ret != 0) return ret;
  ret = clGetDeviceIDs(platform_id, CL_DEVICE_TYPE_GPU, 1, &device_id_,
                       &num_devices);
  if (ret != 0) return ret;
  ...
  input_buffer_ = clCreateBuffer(
      context_, CL_MEM_READ_ONLY,
      sizeof(float) * width_ * height_ * max_elements_, NULL, &ret);
  if (ret != 0) return ret;
  output_buffer_ = clCreateBuffer(context_, CL_MEM_WRITE_ONLY,
                                  sizeof(float) * width_ * height_, NULL, &ret);
  if (ret != 0) return ret;
...
cl_kernel DepthImageAverager::createKernelOpenCL_(
    const std::string &kernel_file, cl_int &ret) {
  ...
  if (pos_max_elements != std::string::npos)
    kernel_source.replace(pos_max_elements,
                          std::string(token_max_elements).size(),
  ...
  if (pos_min_elements != std::string::npos)
    kernel_source.replace(pos_min_elements,
                          std::string(token_min_elements).size(),
                          std::to_string(min_elements_));

  size_t kernel_source_size = kernel_source.length();
  char *tmp = kernel_source.data();
  cl_program program =
      clCreateProgramWithSource(context_, 1, (const char **)&tmp,
...
int DepthImageAverager::computeOpenCL(sensor_msgs::ImagePtr &averaged_image,
  ...
  if (opencl_kernel_ == NULL)
    opencl_kernel_ = createKernelOpenCL_(kernel_file, ret);
  if (ret != 0) return ret;
  ...
  // set the input
  ret = clEnqueueWriteBuffer(command_queue_, input_buffer_, CL_TRUE, 0,
                             sizeof(float) * width_ * height_ * max_elements_,
                             arr_.data(), 0, NULL, NULL);
  if (ret != 0) return ret;

  // run the kernel
  size_t global_item_size = width_ * height_;
  ret = clEnqueueNDRangeKernel(command_queue_, opencl_kernel_, 1, NULL,
