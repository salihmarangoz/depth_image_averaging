...
class DepthImageAverager {
 public:
  ...
  int computeMAD(sensor_msgs::ImagePtr &averaged_image, float mad_upper_limit_a,
                 float mad_upper_limit_b, float mad_scale, bool true_median);

 private:
  void vector2DepthImage_(sensor_msgs::ImagePtr &image,
...
#if USE_OPENCL
 public:
  int computeOpenCL(sensor_msgs::ImagePtr &averaged_image,
...
template <typename T>
void vector2DepthData(sensor_msgs::ImagePtr &image,
                      const std::vector<float> &arr) {
...
template <typename T>
void depthImageStridedCopy(const sensor_msgs::ImageConstPtr &image,
                           std::vector<float> &arr, int stride, int shift) {
